...
/*
vector<SolutionB> optim(vector<Mesh*> observations, int iters, float hybrProb,
	int lambda, int mu){
	// lambda should be a multiple of mu 
	vector<SolutionB> population;
	vector<SolutionB> selected;
	selected.resize(mu);

	for (int p = 0; p < (lambda + mu); p++){
		Solution thisSol (population[0].size());
		thisSol.solMesh = observations[0]->copy();
		population.push_back(thisSol);
		population[p].solMesh->randomize();
		}

	for (int i = 0; i < iters; i++){
		for ( int p = 0; p < population.size(); p++){
			fitness(population[p], observations, 0.3);
			}

		sortSols(population, 0, (population.size()-1));
		vector <Solution>::iterator init = population.begin();
		vector <Solution>::iterator end = population.begin() + mu;
		selected.assign(init, end);
		
		population = selected;

		for (int s = 0; s < selected.size(); s++) {
			for (int u = 0; u < (lambda / mu); u++) {
				Solution newSol;
				int thob = rand() % observations.size();
				perturb(selected[s].solMesh, observations[thob], hybrProb);
